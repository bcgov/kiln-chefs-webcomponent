<script lang="ts">
	...
	$effect(() => {
		...
		if (form) {
			...
			form.authToken = data.authToken;
			if (data.submissionId) {
				form.submissionId = data.submissionId;
			}
			form.endpoints = {
				...
				uploadFile: '/api/file/:submissionId',
				iconsCss:
					CHEFS_SERVER_URL + '/app/webcomponents/v1/assets/font-awesome/css/font-awesome.min.css'
			};
			form.load();
		}
		return () => {
...
<chefs-form-viewer bind:this={formViewer} isolate-styles></chefs-form-viewer>
